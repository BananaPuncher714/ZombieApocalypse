...
public class ZACommands implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		if (commandLabel.equalsIgnoreCase("za") && (args.length == 0 || args[0].equals("commands"))) {
			sender.sendMessage(ChatColor.GREEN + "----Zombie Apocalypse Commands----\n"
		...
		else if (commandLabel.equalsIgnoreCase("za") && args[0].equals("start") && sender.isOp()) // manual start
		{
			...
			if (args.length == 2) { // if they have 2 args, check if what's after start is an integer
				if (isInteger(args[1])) { // if it's an integer
					...
					sender.sendMessage(
							ChatColor.RED + "ZA: Invalid start, the command must be /za start or /za start <integer>.");
				}
				return true;
			}
		...
		else if (commandLabel.equalsIgnoreCase("za") && args[0].equals("start") && !sender.isOp()) // don't start if the
																									// user isnt OP,
																									// yell at them
		{
			...
			{
				sender.sendMessage(ChatColor.GREEN + "ZA: " + ChatColor.DARK_GREEN + main.zkilled + "/" + main.zgoal
						+ ChatColor.GREEN + " zombies killed.");
			} else { // if an apocalypse isn't going on
				sender.sendMessage(
						ChatColor.LIGHT_PURPLE + "ZA: The undead are still sleeping, try again during the apocalypse.");
			}
			return true;
		} else if (commandLabel.equalsIgnoreCase("za") && args[0].equals("getitem")) { // toggle returning the return of
																						// the items names
			if (sender.isOp()) {
